...
## Amendment — TMDB activation readiness & periodic metadata refresh (implemented locally, activation pending)

This amendment records the design that prepares TMDB activation and keeps imported provider-owned metadata fresh. It is **implemented and locally verified**; **production activation remains pending** and `TMDB_ENABLED` stays **false** in hosted production. The owner-controlled activation procedure lives in [`docs/tmdb-activation-rollout.md`](../tmdb-activation-rollout.md).

### Owner-provided TMDB clarification (context, not blanket approval)

The owner supplied evidence that TMDB staff consider **periodically refreshed cached metadata** acceptable and that **embeddings for semantic search** are not "AI training" in the described use case. This is owner-provided clarification for **those specific uses only** — not blanket approval of unrelated uses or commercial licensing, and not a TMDB-mandated refresh interval. No discussion URL or exact TMDB-prescribed interval is asserted.

### Decisions

1. **Gating & attribution reconciled.** The obsolete unconditional "permission pending" restriction is replaced with the documented activation requirements (global `EXTERNAL_CATALOG_ENABLED` + `TMDB_ENABLED` + a configured provider). Default and unconfigured environments stay disabled and make **no** provider request; TMDB credentials stay server-only; provider failures never break local catalog or Open Library results; attribution (notice + `public/tmdb.svg`) is required wherever TMDB content appears.

2. **Bounded periodic refresh worker.** Migrations `20260815120700_refresh_external_media_provider_metadata.sql` and `20260815121000_external_media_refresh_lifecycle.sql` add the `service_role`-only `refresh_external_media(...)`, `mark_external_media_refresh_failed(...)`, and `mark_external_media_removed(...)` RPCs plus `provider_checked_at` / `provider_removed_at` bookkeeping. The operator CLI (`scripts/refresh-catalog.mjs` over the tested `scripts/refresh-catalog-core.ts`) refreshes already-imported provider-owned rows in **deterministic, bounded, oldest-checked-first, resumable** batches with explicit timeouts, bounded concurrency, per-run limits, and bounded retries/backoff honoring `429`/`Retry-After`. A **7-day** freshness target with daily checks is an **engineering default**, not a TMDB interval. Successful checks are tracked separately from content changes: an unchanged successful refresh advances freshness **without** churning content hashes or embeddings, and a failed request never marks a row refreshed. A **write-free dry-run** performs read-only provider requests only.

3. **Canonical identity preserved on refresh.** Refresh only touches provider-owned metadata and preserves media id, slug, aliases, community rating, and all diary/review/list/favorite references. Movie vs. TV kind-qualified identity keeps same-numeric-id titles distinct; curated rows linked to a provider alias are not overwritten.

4. **Authoritative removals vs. transient outages.** A single timeout or `429` is never treated as removal; only a confirmed authoritative removal routes through `mark_external_media_removed(...)`, which suppresses provider content while **retaining** canonical identity and every user reference. No routine hard deletion of media or user-owned rows follows a provider error.

5. **Embedding consistency.** When embedding-input metadata changes, the canonical search document is refreshed and the stale embedding invalidated (all five embedding provenance fields nulled together, guarded by a content-hash mismatch) so an older-hash vector is never stored/served as current and a concurrent re-embed is not clobbered. Only eligible missing/stale embeddings are regenerated via the existing bounded `npm run embed:catalog` tooling; poster-only/operational-timestamp changes do not trigger re-embedding; embedding failure preserves keyword search and leaves the row retry-eligible. TMDB embedding eligibility is a deliberate control (`TMDB_EMBEDDING_ENABLED`), keeping the default embedding-source policy (curated + Open Library) intact until explicitly enabled.

6. **Executable scheduler (owner-gated).** The daily/dispatchable schedule is delivered as an applyable GitHub Actions workflow (a `workflows`-scoped maintainer must add the file; see [scheduler handoff](../ci/catalog-refresh-scheduler-handoff.md)), guarded by a `CATALOG_REFRESH_ENABLED` variable defaulting to **disabled**, with protected secrets, an explicitly confirmed target project (the CLI's `--confirm-project-ref` guard is retained for scheduled writes), minimal permissions, overlap prevention, and bounded refresh followed by bounded stale-embedding backfill.

7. **Safeguards unchanged.** The remote-write guard (both `--allow-remote` and a matching `--confirm-project-ref`), RLS, `service_role`-only EXECUTE, pinned empty search paths, server-only credentials, and no-environment resilience are **not weakened**. No generative AI, bulk clone, new media types, or episode tracking is introduced.
